...
function LoadingState() {
  return (
    <div className="mx-auto flex min-h-[50vh] max-w-[1000px] items-center justify-center px-4 py-20">
      <div className="h-10 w-10 animate-spin rounded-full border-2 border-[#4F46E5] border-t-transparent" />
    </div>
  );
}
...
function MealCard({
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[24px] border border-[#E8EAED] bg-white shadow-[0_4px_24px_rgba(47,51,55,0.06)] transition hover:border-[#4F46E5]/25 hover:shadow-[0_8px_32px_rgba(36,155,96,0.12)]">
      <div className="relative aspect-[16/10] w-full bg-[#F7F7F8]">
        {slotLabel ? (
          <span className="absolute left-3 top-3 z-10 rounded-full bg-white/95 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-[#4F46E5] shadow-sm backdrop-blur-sm">
            {slotLabel}
          </span>
        ) : null}
        {imageUrl ? (
        ...
        {tags.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {tags.map((t) => (
              <span
                key={t}
                className="rounded-full bg-[#EEF2FF] px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[#4F46E5]"
              >
                {t}
              </span>
            ))}
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-[560px] px-4 py-20 text-center">
        ...
        <Link
          href="/"
          className="mt-8 inline-block rounded-full bg-[#4F46E5] px-8 py-3 text-[15px] font-bold text-white hover:bg-[#4338CA]"
        >
          Back to home
        </Link>
      </div>
    ...
    <div className="bg-white pb-20 pt-28 sm:pt-32">
      <div className="mx-auto max-w-[1000px] px-4 sm:px-6">
        ...
        <div className="mt-14 flex flex-wrap justify-center gap-4">
          <Link
            href="/plans"
            className="inline-flex items-center justify-center rounded-full bg-[#4F46E5] px-8 py-3.5 text-[15px] font-bold text-white shadow-sm hover:bg-[#4338CA]"
          >
